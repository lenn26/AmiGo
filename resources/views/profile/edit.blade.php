            ...
            <form method="post" action="{{ route('profile.update') }}" class="p-4 sm:p-8 bg-white shadow rounded-xl" enctype="multipart/form-data">
                ...
                <div class="flex justify-between items-start mb-8">
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            @if(Auth::user()->is_verified)
                                <svg class="w-8 h-8 text-blue-500" viewBox="0 0 20 20" fill="currentColor" title="Compte étudiant vérifié">
                        ...
                        <!-- Icône vérifiée avec texte -->
                        @if(Auth::user()->is_verified)
                            <p class="mt-2 text-sm text-blue-500 font-medium flex items-center gap-1">
                                <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                </svg>
                                Compte étudiant vérifié
                            </p>
                        @endif
                    </div>
                    <div class="flex flex-col items-center" x-data="{ 
                        ...
                        <div class="w-24 h-24 bg-blue-500 rounded-full flex items-center justify-center text-white text-4xl mb-2 overflow-hidden relative">
                            <img x-show="previewUrl && previewUrl !== ''" :src="previewUrl" alt="Profile Photo" class="w-full h-full object-cover absolute inset-0">
                            <svg x-show="!previewUrl || previewUrl === ''" class="w-12 h-12 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                            </svg>
                        </div>
                        <input type="file" name="avatar" x-ref="fileInput" @change="previewImage" accept="image/*" class="hidden">
                        <button type="button" @click="selectFile()" class="text-blue-500 text-sm hover:underline">Modifier photo</button>
                    </div>
                </div>
